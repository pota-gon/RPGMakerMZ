...
・アップデート情報
* Ver1.0.2
- MP消費率を割合消費に含めるかを指定できる機能追加
- 変数コスト機能追加
- 通常攻撃コスト0時にアイテムが無条件で使用可能になるバグ修正
...
HP、MP、TP、所持金、アイテム、変数など多様なコストを設定できます。
...
- MP消費率を含める設定をONにすると、スキルに設定されたMP消費率(%)も割合消費として計算されます

@param IncludeMpRate
@type boolean
@text MP消費率を含める
@desc スキルのMP消費率(%)を割合消費の計算に含めるか
@on 含める
@off 含めない
@default false
...
@param VariableCostMetaName
@text 変数消費タグ
@desc 変数消費に使うメモ欄タグの名称
デフォルトは 変数消費
@default 変数消費

@param VariableRateCostMetaName
@text 変数割合消費タグ
@desc 変数割合消費に使うメモ欄タグの名称
デフォルトは 変数割合消費
@default 変数割合消費

@param VariableCosts
@type struct<VariableCost>[]
@text 変数コスト設定
@desc 変数コストの設定リスト
@default ["{\"VariableId\":\"1\",\"VariableName\":\"V1\",\"CostColor\":\"0\"}","{\"VariableId\":\"2\",\"VariableName\":\"V2\",\"CostColor\":\"0\"}","{\"VariableId\":\"3\",\"VariableName\":\"V3\",\"CostColor\":\"0\"}"]
*/
...
(() => {
    ...
    const IncludeMpRate = Potadra_convertBool(params.IncludeMpRate);
    ...
    const VariableCostMetaName = String(params.VariableCostMetaName || '変数消費');
    const VariableRateCostMetaName = String(params.VariableRateCostMetaName || '変数割合消費');

    // 変数コスト用パラメータ
    const variableCosts = JSON.parse(params.VariableCosts || '[]');
    const VariableColorMap = {};
    const VariableNameMap = {};
    
    for (const cost of variableCosts) {
        const variableCost = JSON.parse(cost);
        const variableId = Number(variableCost.VariableId);
        VariableColorMap[variableId] = Number(variableCost.CostColor || 0);
        VariableNameMap[variableId] = String(variableCost.VariableName || 'V');
    }
     ...
     * @returns {} 
     */
    ColorManager.variableCostColor = function (variableId) {
        return this.textColor(VariableColorMap[variableId] || 0);
    };
    ...
    function attackCostZero(item, battler) {
        return AttackCostZero && item && item.stypeId && item.id === battler.attackSkillId();
    }
     ...
     *
     * @param {} meta - メタ情報
     * @param {number} param - 現在の値
     * @param {number} cost - コスト
     * @param {string} meta_name - メタタグ名
     * @param {string} rate_meta_name - 割合消費メタタグ名
     * @param {string} max_meta_name - 最大値割合消費メタタグ名
     * @param {object} skill - スキル情報（MP消費率チェック用）
     */
    function canPaySkillCommonCost(meta, param, cost, meta_name, rate_meta_name, max_meta_name, skill = null) {
        ...
        // MP消費率を含める設定がONで、スキル情報があり、MP消費率が設定されている場合
        const has_mp_rate = IncludeMpRate && skill && skill.mpCost > 0 && rate_meta_name === MpRateCostMetaName;
        
        if (rate_cost || has_mp_rate) {
    ...
    Game_BattlerBase.prototype.skillMpCost = function (skill) {
        ...
        // MP消費率を含める設定がONの場合も、基本のMP消費計算は_Game_BattlerBase_skillMpCostで行われる
        // skill.mpCostが0より大きい場合、それは既に最大MPに対する割合として計算されている
    ...
    Game_BattlerBase.prototype.skillVariableCost = function (skill) {
        ...
        // 設定された変数のみチェック
        for (const costStr of variableCosts) {
            const variableCost = JSON.parse(costStr);
            const id = Number(variableCost.VariableId);
            
            const variable_cost = Potadra_meta(skill.meta, VariableCostMetaName + id);
            const variable_rate_cost = Potadra_meta(skill.meta, VariableRateCostMetaName + id);
            
            if (variable_cost || variable_rate_cost) {
                variableId = id;
                
                if (variable_cost) { // 変数消費
                    cost += Number(variable_cost || 0);
                }
                if (variable_rate_cost) { // 変数割合消費
                    cost += ceilZeroCost($gameVariables.value(id), variable_rate_cost, cost);
                }
                
                break; // 最初に見つかった変数のみ使用
            }
        }
    ...
    Game_BattlerBase.prototype.paySkillCost = function (skill) {
        ...
        if (this._variableId > 0) {
            const variable_cost = this.skillVariableCost(skill);
            $gameVariables.setValue(this._variableId, $gameVariables.value(this._variableId) - variable_cost);
        }
    };
    ...
    Game_BattlerBase.prototype.canPaySkillCost = function (skill) {
        ...
        return (
            canPaySkillCommonCost(meta, this._mp, this.skillMpCost(skill), null, MpRateCostMetaName, MaxMpRateCostMetaName, skill) &&
    ...
    Game_BattlerBase.prototype.canPaySkillVariableCost = function (skill) {
        const cost = this.skillVariableCost(skill);
        ...
        for (const costStr of variableCosts) {
            ...
            if (variable_rate_cost) {
                return $gameVariables.value(id) > 0 && $gameVariables.value(id) >= cost;
            } else if (variable_cost) {
                return $gameVariables.value(id) >= cost;
            }
        }
    ...
    Window_SkillList.prototype.drawSkillCost = function (skill, x, y, width) {
        ...
        } else if (this._actor.skillVariableCost(skill) > 0) {
            const variableId = this._actor._variableId;
            this.changeTextColor(ColorManager.variableCostColor(variableId));
            const variableName = VariableNameMap[variableId] || 'V';
            this.drawText(variableName + this._actor.skillVariableCost(skill) + '/' + $gameVariables.value(variableId), x, y, width, "right");
        }
    };
    ...
    Window_SkillList.prototype.costSkillWidth = function (skill) {
        ...
        if (this._actor.skillHpCost(skill) > 0 && this._actor.skillMpCost(skill) > 0 && this._actor.skillTpCost(skill) > 0) {
            ...
            const variableId = this._actor._variableId;
            const variableName = VariableNameMap[variableId] || 'V';
            cost = variableName + this._actor.skillVariableCost(skill) + '/' + $gameVariables.value(variableId);
        }
        return this.textWidth(cost);
    };
})();
