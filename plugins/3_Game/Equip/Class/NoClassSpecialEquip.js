/*:
@plugindesc
職業装備禁止 Ver1.0.1(2026/1/10)

@url https://raw.githubusercontent.com/pota-gon/RPGMakerMZ/refs/heads/main/plugins/3_Game/Equip/Class/NoClassSpecialEquip.js
@target MZ
@author ポテトードラゴン
...
@help
## 概要
指定した職業が装備できない装備をメモから設定できます

## 使い方
特定の職業だけが装備できない装備を作成したい場合に使用します

1. 装備禁止にしたい武器または防具を作成します
2. メモ欄に <職業以外: ID OR 名前> のように設定します
...
(() => {
    ...
    // 各パラメータ用定数
    const NoClassMetaName = String(params.NoClassMetaName || '職業以外');

    // 職業装備禁止判定
    function noClassSpecialEquip(meta, klass) {
        const classes = Potadra_metaData(meta[NoClassMetaName]);
        if (classes) {
            for (const class_name of classes) {
                const class_id = Potadra_checkName($dataClasses, class_name);
                if (class_id === klass.id) {
                    return false; // 指定された職業は装備不可
                }
            }
        }
        return true; // 指定されていない職業は装備可能
    }
    ...
    Game_BattlerBase.prototype.canEquipWeapon = function(item) {
        ...
        // 職業装備禁止判定
        return noClassSpecialEquip(item.meta, this.currentClass());
    };
