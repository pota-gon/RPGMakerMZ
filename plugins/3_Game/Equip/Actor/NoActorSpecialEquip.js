/*:
@plugindesc
アクター装備禁止 Ver1.0.1(2026/1/10)

@url https://raw.githubusercontent.com/pota-gon/RPGMakerMZ/refs/heads/main/plugins/3_Game/Equip/Actor/NoActorSpecialEquip.js
@target MZ
@author ポテトードラゴン
...
@help
## 概要
指定したアクターが装備できない装備をメモから設定できます

## 使い方
特定のアクターだけが装備できない装備を作成したい場合に使用します

1. 装備禁止にしたい武器または防具を作成します
2. メモ欄に <アクター以外: ID OR 名前> のように設定します
   ※ 複数指定したい場合は、以下のように改行してください
   <アクター以外: 1
   2
リード>
3. 装備の判定をするときに指定したアクターは装備できなくなります

@param NoActorMetaName
@text アクター装備禁止タグ
@desc アクター装備禁止に使うメモ欄タグの名称
デフォルトは アクター以外
@default アクター以外
*/
(() => {
    ...
    // 各パラメータ用定数
    const NoActorMetaName = String(params.NoActorMetaName || 'アクター以外');

    // アクター装備禁止判定
    function noActorSpecialEquip(meta, actor) {
        const actors = Potadra_metaData(meta[NoActorMetaName]);
        if (actors) {
            for (const actor_name of actors) {
                const actor_id = Potadra_checkName($dataActors, actor_name);
                if (actor_id === actor.actorId()) {
                    return false; // 指定されたアクターは装備不可
                }
            }
        }
        return true; // 指定されていないアクターは装備可能
    }
    ...
    Game_BattlerBase.prototype.canEquipWeapon = function(item) {
        ...
        // アクター装備禁止判定
        return noActorSpecialEquip(item.meta, this);
    };
